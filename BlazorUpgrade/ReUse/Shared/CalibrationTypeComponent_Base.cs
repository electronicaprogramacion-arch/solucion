...
namespace CalibrationSaaS.Infraestructure.Blazor.Shared
{
    ...
    public partial class CalibrationTypeComponent<Entity,CalibrationItem, CalibrationItemResult> : FComponentBase 
        where Entity: class, IResolution, Domain.Aggregates.Interfaces.IConfiguration,ITolerance ,
        ...
        public  Task CloseGenericWindow2(string CalibrationType= "")
        {
            ...
            if (list != null)
            {
                ...
                foreach (var item in list)
                {
                    ...
                    {
                        genericCalibration2 = Selectlist.Where(x => x.Position == item.Position && x.CalibrationSubTypeId==item.CalibrationSubTypeId).FirstOrDefault();
