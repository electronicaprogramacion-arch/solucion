...
{
    public class TestCode_SearchBase : Base_Create<TestCode, Application.Services.IWorkOrderDetailServices<CallContext>, Domain.Aggregates.Shared.Basic.AppState>
    //ComponentBase,IPage<Manufacturer, Application.Services.IBasicsServices<CallContext>, AppState>
    {
        ...
        public ResponsiveTable<TestCode> Grid { get; set; } = new ResponsiveTable<TestCode>();
        public List<TestCode> ListMan = new List<TestCode>();
        ...
        public override async Task<ResultSet<TestCode>> LoadData(Pagination<TestCode> pag)
        {
            WorkOrderDetailGrpc basics = new WorkOrderDetailGrpc(Client);

            var Eq = (await basics.GetTestCodes(pag, new CallContext()));
        ...
        public async Task<bool> Delete(TestCode DTO)
        {
            //await searchComponent.ShowModalAction();

            await Client.DeleteTestCode(DTO, new CallOptions());
