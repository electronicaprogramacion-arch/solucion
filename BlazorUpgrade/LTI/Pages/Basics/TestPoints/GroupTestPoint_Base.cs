...
namespace CalibrationSaaS.Infraestructure.Blazor.Pages.Basics.TestPoints
{
    ...
    {
        if (Eq != null && Eq.UnitofmeasurementID.HasValue && Group != null && Eq.EquipmentTemplateID == 0 && Eq.UnitofmeasurementID.HasValue)
        {
            //if (Group.OutUnitOfMeasurementID != Eq.UnitofmeasurementID.Value)
            //{
    ...
    TestPoint RowExceute(TestPoint t)
    {
    ...
    TestPoint NewItem()
    {
        ...
        t.UnitOfMeasurementOutID = Group.OutUnitOfMeasurementID;
    ...
    public TestPoint TestPoint { get; set; }
        ...
        void Change2(ChangeEventArgs arg)
        {
    ...
    void Change(TestPoint TestPoint, ChangeEventArgs arg)
    {
        var val = arg.Value.ToString();
        decimal value;
        var obj = decimal.TryParse(arg.Value.ToString(), out value);
    ...
    async Task EditGrid(ChangeEventArgs arg)
    {
    ...
    string Bound(TestPoint TestPoint)
    {
        if (TestPoint == null)
        {
            return "";
    ...
    public async Task Generate(TestPointGroup test)
    {
        ...
        //Calculate.Conversion<UnitOfMeasure>(test., arg, AppState.UnitofMeasureList, nameof(test.UnitOfMeasurementOut.UnitOfMeasureID));
         ...
         var CapacityReal = QueryableExtensions2.ConversionUOM(Eq.UnitofmeasurementID, Capacity, test.OutUnitOfMeasurementID,AppState.UnitofMeasureList);
