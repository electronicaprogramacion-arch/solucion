...
{
    public class SelectPOE_SearchBase : Base_Create<PieceOfEquipment, Func<dynamic, Application.Services.IPieceOfEquipmentService<CallContext>>, Domain.Aggregates.Shared.AppStateCompany>
    ...
    //Application.Services.IBasicsServices, Domain.Aggregates.Shared.Basic.AppState>
    {
        ...
        public override async Task<ResultSet<PieceOfEquipment>> LoadData(Pagination<PieceOfEquipment> pag)
        {
           ...
           //yp pag.Entity.EquipmentTemplate.EquipmentTypeObject.CalibrationTypeID = CalibrationTypeID;
            ...
            //{
            //    ResultSet<PieceOfEquipment> result = new ResultSet<PieceOfEquipment>();
            //    var _poe = Eq.List.Where(x => x.DueDate >= DateTime.Now);

            //    result.List = _poe.ToList();
            //    result.CurrentPage = 1;
            //    result.Count = _poe.ToList().Count;
            //    //result.PageTotal =
            //    result.PageTotal = 1;
            //    result.Shown = _poe.ToList().Count;
            //    return result;
            //}
            //else
            //{
