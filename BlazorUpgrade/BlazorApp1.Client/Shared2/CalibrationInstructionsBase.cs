...
namespace BlazorApp1.Blazor.Shared
{
    ...
    {
        public BlazorApp1.Blazor.Blazor.Pages.Basics.ResolutionComponent ResolutionComponent { get; set; }
        ...
        protected override async Task OnInitializedAsync()
        {
            ...
            //if (compositeWodList != null && compositeWodList.Count > 0)
            //{
              
            //    // Try to get the full WorkOrderDetail list from WorkOrderItemCreate
            //    var listWods = WorkOrderItemCreate?.listWods;

            //    foreach (var item in compositeWodList)
            //    {
            //        Console.WriteLine($"DEBUG: Processing item {item?.WorkOrderDetailID}");
            //        // Find the corresponding WorkOrderDetail for more descriptive text
            //        var value = item?.PieceOfEquipment?.EquipmentTemplate?.Model + "-" + item?.PieceOfEquipment?.SerialNumber;
            //        KeyValueOption key = new KeyValueOption()
            //        {
            ...
            //    }
            //    Console.WriteLine($"DEBUG: compositeWods final count: {compositeWods.Count}");
            //}
            //else
            //{
            //    Console.WriteLine("DEBUG: Skipping compositeWodList block - condition is false");
            //}
