...
namespace BlazorApp1.Blazor.Blazor.LTI
{
    ...
    public async Task AddItem(object e, WeightSet add)
    {
        ...
        try
        {
                DesSelectItem =    new List<WeightSet>();
                SelectItem = new List<WeightSet>();
                bool remove = false;
            bool additem = false;
                ...
                {
                    int index = 0;
                    int conts = DataSource.Count - 1;//SelectItem.Count() - 1;
                    for (int i = 0; i <= conts; i++)
                    {
                        index = 0;
                        if (SelectItem.Count > i)
                        SelectItem.Remove(SelectItem[i]);
                        conts = conts - 1;
                        //DesSelectItem.Add(SelectItem[i]);
                    }
                    SelectItem.Add(add);
                }
