...
{
    public class WorkOrderDetailOff2_SearchBase : Base_Create<WorkOrderDetail, CalibrationSaaS.Application.Services.IWorkOrderDetailServices<CallContext>, CalibrationSaaS.Domain.Aggregates.Shared.AppStateCompany>
    //,IPage<WorkOrder, Application.Services.IAssetsServices<CallContext>, Domain.Aggregates.Shared.Basic.AppState>
    {
        ...
        [Inject]
        public CalibrationSaaS.Application.Services.ICustomerService<CallContext> CustomerServices { get; set; }


        [Inject] public CalibrationSaaS.Application.Services.IWorkOrderDetailServices<CallContext> _wodServices { get; set; }

        [Inject]
        public CalibrationSaaS.Application.Services.IBasicsServices<CallContext> BasicsServices { get; set; }




        [Inject]
        public CalibrationSaaS.Application.Services.IAssetsServices<CallContext> AssetsServices { get; set; }

        [Inject]
        public CalibrationSaaS.Application.Services.IUOMService<CallContext> UOMServices { get; set; }
        ...
        [Inject]
        public Func<dynamic, CalibrationSaaS.Application.Services.ISampleService2<CallContext>> OffLineClient { get; set; }

        [Inject]
        Func<dynamic, CalibrationSaaS.Application.Services.IBasicsServices<CallContext>> BasicsServices2 { get; set; }


         [Inject]
        Func<dynamic, CalibrationSaaS.Application.Services.IWorkOrderDetailServices<CallContext>> Client2 { get; set; }


        public List<WorkOrderDetail> List = new List<WorkOrderDetail>();

        public List<CalibrationSaaS.Domain.Aggregates.Entities.Status> _statusList = new List<CalibrationSaaS.Domain.Aggregates.Entities.Status>();
...
#pragma warning restore CS1998 // El m�todo asincr�nico carece de operadores "await" y se ejecutar� de forma sincr�nica. Puede usar el operador 'await' para esperar llamadas API que no sean de bloqueo o 'await Task.Run(...)' para hacer tareas enlazadas a la CPU en un subproceso en segundo plano.
        ...
        public List<WorkOrderDetail> WODOnlineList { get; set; }// = new List<WorkOrderDetail>();
...
#pragma warning restore CS0108 // 'WorkOrderDetailOff_SearchBase.Dispose()' oculta el miembro heredado 'Base_Create<WorkOrderDetail, IWorkOrderDetailServices<CallContext>, AppStateCompany>.Dispose()'. Use la palabra clave new si su intenci�n era ocultarlo.
            ...
            public async Task NewItemOff()
        {
            ...
            else if (ConnectionStatusService.GetCurrentStatus() && !ConnectionStatusService.GetCurrentConnected())
            {
                try
                {
                    var sts = await wod.GetStatus();

                     ConnectionStatusService.ConnectedChangeStatus(true,false,true);
                }
                catch (Exception ex)
                {
                    await ShowToast("To upload to the server, verify that it is in Online mode", ToastLevel.Warning);
                    return;
                }
        ...
        public async Task NewItem()
        {
            ...
            {            
                ...
                var listfilter = Grid.Items;

                if(Grid != null && Grid.CheckList != null)
                {
                    //foreach (var itemx in Grid.CheckList)
                    //{
                    //    await addToListPoES2(itemx);
                    //}
                }
        ...
        protected  override async Task OnInitializedAsync()
        {
            ...
            {
                throw new Exception("Offline database not exists");
            }
