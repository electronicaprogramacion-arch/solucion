...
{
    public class POEWO_MultiSelectPerBase : Base_Create<PieceOfEquipment, CalibrationSaaS.Application.Services.IPieceOfEquipmentService<CallContext>, CalibrationSaaS.Domain.Aggregates.Shared.AppStateCompany>
    ...
    //Application.Services.IBasicsServices, Domain.Aggregates.Shared.Basic.AppState>
    {
        ...
        //private CalibrationSaaS.Domain.Aggregates.ValueObjects.Pagination<PieceOfEquipment> pag = new CalibrationSaaS.Domain.Aggregates.ValueObjects.Pagination<PieceOfEquipment>();
        ...
        public override async Task<ResultSet<PieceOfEquipment>> LoadData(Pagination<PieceOfEquipment> pag)
        {
            ...
            var Eq = (await _poeGrpc.GetAllPeripheralsPag(pag));


            // var QS = (from std in Eq.List
            //           select std.PieceOfEquipmentID).Except(Parameter1.Select(y => y.PieceOfEquipmentId)).ToList();

            // List<PieceOfEquipment> list = new List<PieceOfEquipment>();
            //foreach(var item in Eq.List)
            // {
            //     var aaa = Parameter1.Where(x => x.PieceOfEquipmentId == item.PieceOfEquipmentID).FirstOrDefault();
            //     if (aaa == null)
            //     {
            //         list.Add(item);
            //     }
            // }
            // int total = QueryableExtensions2.GetTotalPages(list.Count, Eq.Shown);
            // Eq.List = list;
            // Eq.Count = list.Count;
            // Eq.PageTotal = total;
