...
{
    public class GetTestPointBased : ComponentBase, IGetItems<WorkOrderDetail,GenericCalibration2>
    {
        public async Task<List<GenericCalibration2>> GetItems(WorkOrderDetail eq, int CalibrationSubTypeId, int ihneritCalSubtypeId)
        {
            ...
            var items2 = await new CalibrationSaaS.Infraestructure.Blazor.GenericMethods.CreateTestPointsBased().CreateItems(eq, CalibrationSubTypeId);
