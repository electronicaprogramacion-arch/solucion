...
using BlazorApp1.Blazor.LTI.Scale;//CalibrationSaaS.Infraestructure.Blazor.LTI.Scale.WeightSetComponent;
using Helpers;
...
namespace CalibrationSaaS.Infraestructure.Blazor.LTI
{
    public class LinearityComponentBase : KavokuComponentBase<WorkOrderDetail>
    {
        ...
        public List<Linearity> OrderItems(List<Linearity> Items)
        {
        ...
        public async Task SelectWeight(Linearity item, int position)
        {
            ...
            if (WeightSetList != null)
            {
                ...
                */
                ...
                // NEW BEHAVIOR: Only update the selected item's weights
                if (item.WeightSets == null)
                {
                    item.WeightSets = new List<WeightSet>();
                }
                ...
                // Only calculate weight for the selected item
                double suma = 0;
                if (item.WeightSets != null && item?.TestPoint?.NominalTestPoit != 0)
                {
                    foreach (var ws in item.WeightSets)
                    {
                        suma = suma + ws.WeightNominalValue;
                    }

                    item.BasicCalibrationResult.WeightApplied = Math.Round(r.CalculateWeight, decimalNumber);
                }
                else
                {
                    item.BasicCalibrationResult.WeightApplied = 0;
                }
