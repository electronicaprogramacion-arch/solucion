...
using BlazorApp1.Blazor.LTI.Scale;

namespace CalibrationSaaS.Infraestructure.Blazor.LTI.BalanceAndScale
{
    public class EccentricityComponentBase : KavokuComponentBase<WorkOrderDetail>
    {
        ...
        public Domain.Aggregates.Entities.Eccentricity Entity { get; set; } = new Domain.Aggregates.Entities.Eccentricity();
        ...
        protected override async Task OnInitializedAsync()
        {
            ...
            else
            {
               ...
               && WorkOrderItemCreate?.eq?.PieceOfEquipment?.EquipmentTemplate?.TestGroups?.ElementAtOrDefault(0)?.TestPoints
               .Where(x => x.CalibrationType.ToLower() == "eccentricity").FirstOrDefault() != null
                ...
                && WorkOrderItemCreate?.eq?.PieceOfEquipment?.TestGroups?.ElementAtOrDefault(0)?.TestPoints
                .Where(x => x.CalibrationType.ToLower() == "eccentricity").FirstOrDefault() != null
                )
                {
                    //Console.WriteLine("CreateEccentricityList");
