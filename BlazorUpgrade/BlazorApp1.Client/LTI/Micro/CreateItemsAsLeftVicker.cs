...
{
    public class CreateItemsAsLeftVicker : ComponentBase, ICreateItems
    {
            ...
            if (eq.WOD_Weights != null && eq.WOD_Weights.Count > 0) 
            {
                ...
                foreach (var item in eq.WOD_Weights)
                {
                    ...
                    if (poe.TypeMicro.HasValue && poe.TypeMicro.Value==1) 
                    {
                        ...
                        gc.BasicCalibrationResult.Position = cont;
                        gc.BasicCalibrationResult.Resolution = poe.Resolution;
