...
namespace CalibrationSaaS.Infraestructure.Blazor.LTI.LaserMicrometer.AsLeft
{
    public class Get : ComponentBase, IGetItems<WorkOrderDetail,GenericCalibration2>
    {
        public async Task<List<GenericCalibration2>> GetItems(WorkOrderDetail eq, int CalibrationSubTypeId, int ihneritCalSubtypeId)
        {
            ...
            var items2 = await new CalibrationSaaS.Infraestructure.Blazor.LTI.LaserMicrometer.AsLeft.Create().CreateItems(eq, CalibrationSubTypeId);
