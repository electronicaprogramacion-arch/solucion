...
{
    public partial  class ASTMRockAsleftComponent<CalibrationItem, CalibrationItemResult> 
         ...
         where CalibrationItem : Rockwell, new() where CalibrationItemResult : RockwellResult, new()
    {
        ...
        public WorkOrderItemCreate WorkOrderItemCreate { get; set; }
...
#pragma warning restore CS0168 // La variable 'ex' se ha declarado pero nunca se usa
...

        {
            ...
            Console.WriteLine(RefreshResolution);
            if (WorkOrderItemCreate.eq.CalibrationTypeID.HasValue && ValidateGrid(WorkOrderItemCreate.eq, IsCompresion))
            {
...
#pragma warning restore CS4014 // Como esta llamada no es 'awaited', la ejecución del método actual continuará antes de que se complete la llamada. Puede aplicar el operador 'await' al resultado de la llamada.
                ...
                if (WorkOrderItemCreate.eq.CalibrationTypeID.HasValue && !ValidateGrid(WorkOrderItemCreate.eq, IsCompresion))
            {
                ...
                foreach (var itemr in result.List)
                {
                    ...
                    cont = cont + 1;
                    var rock3 = new Rockwell();
                    rock3.BasicCalibrationResult = new RockwellResult();

                    rock3.BasicCalibrationResult.ScaleRange = itemr.Scale + "-MID";

                    rock3.BasicCalibrationResult.Position = cont;
                    rock3.BasicCalibrationResult.SequenceID = cont;
                    rock3.SequenceID = cont;

                    rock3.CalibrationSubTypeId = cs;
        ...
        public async Task ExecuteFormula()
        {
            try
            {
                ...
                foreach (var item in aa)
                {
                    ...
                    sequence.Add(item.BasicCalibrationResult.Test3);
                    sequence.Add(item.BasicCalibrationResult.Test4);
                    sequence.Add(item.BasicCalibrationResult.Test5);
                    ...
                    //sequence2.Add(Math.Abs(item.BasicCalibrationResult.Nominal - sequence[1]));
        ...
        public async Task SelectWeight(CalibrationItem item, int position)
        {
            ...
            if (WeightSetList?.Count > 0)
            {
                ...
                //var max = CalResult + poeselect.ToleranceValue;

                //var min = CalResult - poeselect.ToleranceValue;
