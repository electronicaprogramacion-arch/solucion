...
{
    public class CreateItemsAsLeft : ComponentBase, ICreateItems<WorkOrderDetail, GenericCalibration2>
    {
        ...
        public async Task<List<GenericCalibration2>> CreateItems(WorkOrderDetail eq, int CalibrationSubTypeId, string GroupName = null)
        {
            ...
            {
                throw new Exception("Mass Not Configured");
            }

            var sr = eq.PieceOfEquipment.TestPointResult.Where(x => x.CalibrationSubTypeId == (CalibrationSubTypeId - 1));
            ...
            if(pieces.Count> 0)
            {
                var resultado = await _poeGrpc.GetResolutionByMass(pieces);
                ...
                foreach (var item in resultado)
                {
                    ...
                    gce.Mass = item.Capacity;
                    gce.UoM= item.UnitOfMeasureID;
                    ...
                    gce.MPE= item.Tolerance.ToleranceValue + " " + item.ToleranceUoM.GetUoM(AppState.UnitofMeasureList).Abbreviation;
