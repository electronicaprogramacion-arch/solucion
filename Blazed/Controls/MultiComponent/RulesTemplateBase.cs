...
#pragma warning restore CS1998 // El método asincrónico carece de operadores "await" y se ejecutará de forma sincrónica. Puede usar el operador 'await' para esperar llamadas API que no sean de bloqueo o 'await Task.Run(...)' para hacer tareas enlazadas a la CPU en un subproceso en segundo plano.
        ...
        public async Task<bool> IsInPolicy(Policies policy)
        {
            ...
            var user = (await stateTask).User;
                string policy2 = "HasAccess";
                if (string.IsNullOrEmpty(MultiComponent?.Policy))
                {
                    policy2 = "HasAccess";
                }

                var ed = await AuthorizationService.AuthorizeAsync(user, null, policy2);
            ...
            {
                //Console.WriteLine("IsInPolicy :" + ex.Message);
             return false;
            }  
