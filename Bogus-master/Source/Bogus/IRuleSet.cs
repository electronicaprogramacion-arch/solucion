...
namespace Bogus
{
   ...
   public interface IRuleSet<T> 
   {
      ...
      /// </summary>
      Faker<T> CustomInstantiatorAsync(Func<Faker, Task<T>> factoryMethod);
      /// <summary>
      /// Uses the factory method to generate new instances.
      /// </summary>
      ...
      Faker<T> RuleFor<TProperty>(Expression<Func<T, TProperty>> property, Func<Faker, T, Task<ViewProperty<TProperty>>> setter);

      Faker<T> RuleFor2<TProperty>(Expression<Func<T, TProperty>> property, Func<Faker, T, Task<ViewProperty<TProperty>>> setter,
        Expression<Func<T, bool>> expression,Func<Faker, T, Task<ViewProperty<TProperty>>> fault=null);

      //Faker<T> RuleFor<TProperty>(Expression<Func<T, TProperty>> property, string MethodName = "", params Func<Faker, T, Task<ViewProperty<TProperty>>>[] prop);
