...
namespace CalibrationSaaS.Infraestructure.Grpc.Services
{
    ...
    public class CustomerService : ICustomerService<CallContext>
    {
        ...
        public async ValueTask<Customer> CreateCustomer(Customer customerDTO, CallContext context = default)
        {
            ...
            return await GetCustomersByID(customerDTO, context);
        }
        ...
        public async ValueTask<ICollection<Address>> GetAddressesAsync(CallContext context)
        {
            var result = await customerLogic.GetAddressesAsync();
            return result;
        }

        public async ValueTask<Address> GetAddressesByIDAsync(Address Address, CallContext context)
        {
            var result = await customerLogic.GetAddressesByIDAsync(Address);
            return result;
        }

        public async ValueTask<ResultSet<Address>> GetAddress(Pagination<Address> Pagination, CallContext context)
        {
            var result = await customerLogic.GetAddress(Pagination);
            return result;
        }

        public async ValueTask<ResultSet<AddressCustomerViewModel>> GetAddressCustomer(Pagination<AddressCustomerViewModel> Pagination, CallContext context)
        {
            var result = await customerLogic.GetAddressCustomer(Pagination);
            return result;
        }

        public async ValueTask<ResultSet<AddressCustomerViewModel>> GetAddressCustomerOptimized(Pagination<AddressCustomerViewModel> Pagination, CallContext context)
        {
            var result = await customerLogic.GetAddressCustomerOptimized(Pagination);
            return result;
        }

        public async ValueTask<Customer> ReplaceCustomer(CustomerReplaced DTO, CallContext context)
        {
            var result = await customerLogic.ReplaceCustomer(DTO);
            return result;
        }

        public async ValueTask<IEnumerable<Contact>> GetContactsByCustomID(Customer customer, CallContext context)
        {
            var result = await customerLogic.GetContactsByCustomID(customer);
            return result;
        }

        public async ValueTask<Contact> GetContactByEmail(string email, CallContext context)
        {
            var result = await customerLogic.GetContactByEmail(email);
            return result;
        }
    }
}
