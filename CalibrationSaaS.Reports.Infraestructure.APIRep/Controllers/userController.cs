...
namespace IntegrationApi.Controllers
{
    ...
    public class userController : ControllerBase
    {
        ...
        public userController(ILogger<userController> logger,CalibrationSaaS.Application.UseCases.BasicsUseCases _BasicsUseCases )
        {
            _logger = logger;
        ...
        public async Task<ActionResult<IEnumerable<User>>> Get()
        {
            ...
            try
            {
                ...
                UserDomain domain = new UserDomain(basicsUseCases);

               
                //foreach (var item2 in real)
                //{
                //    UserViewModel us = new UserViewModel();
                //    us.name = item2.UserName;
                //    var ass = await domain.CreateUser(us);
                //}
...
return BadRequest("Unable to create, check the information sent, Customer null");
        ...
        public async Task<ActionResult<string>> PostCustomer([FromBody] UserViewModel user)
        {
            ...
            try
            {
                ...
                var ass = await domain.CreateUser(user);
