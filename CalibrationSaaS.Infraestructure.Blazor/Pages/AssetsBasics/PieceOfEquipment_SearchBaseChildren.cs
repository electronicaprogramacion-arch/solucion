...
{
    public class PieceOfEquipment_SearchBaseChildren : Base_Create<PieceOfEquipment, Func<dynamic, Application.Services.IPieceOfEquipmentService<CallContext>>,
    ...
    //Application.Services.IBasicsServices, Domain.Aggregates.Shared.Basic.AppState>
    {
        ...
        [Inject] public Func<dynamic, Application.Services.IBasicsServices<CallContext>> _basicServices { get; set; }
        ...
        Pagination<PieceOfEquipment> pag2;
        public override async Task<ResultSet<PieceOfEquipment>> LoadData(Pagination<PieceOfEquipment> pag)
        {
            ...
            if (IsForEt == false && !string.IsNullOrEmpty(ParentID))
            {
                ...
                pag.Entity.ParentID = ParentID;
                result = (await _poeGrpc.GetPiecesOfEquipmentChildren(pag));
