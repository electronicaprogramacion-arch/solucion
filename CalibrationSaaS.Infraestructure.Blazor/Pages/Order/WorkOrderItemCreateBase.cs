...
namespace CalibrationSaaS.Infraestructure.Blazor.Pages.Order
{
    public class WorkOrderItemCreateBase :
        Base_Create<CalibrationSaaS.Domain.Aggregates.Entities.WorkOrderDetail,
            Func<dynamic, CalibrationSaaS.Application.Services.IWorkOrderDetailServices<CallContext>>, CalibrationSaaS.Domain.Aggregates.Shared.AppStateCompany>, IWorkOrderItemCreate
    {

        public ChangeEventArgs StandardsToAssing { get; set; } = new ChangeEventArgs();


        public CalibrationSaaS.Infraestructure.Blazor.Pages.AssetsBasics.WorkOrderDetailChildren_Search WorkOrderDetailSearch
            = new CalibrationSaaS.Infraestructure.Blazor.Pages.AssetsBasics.WorkOrderDetailChildren_Search();
        ...
        public CalibrationSaaS.Infraestructure.Blazor.Shared.CalibrationTypeComponent<WorkOrderDetail, GenericCalibration2, GenericCalibrationResult2> CalibrationTypeComponent { get; set; }
...
#pragma warning restore CS0108 
        ...
        //[Inject]
        //public Application.Services.IBasicsServices<CallContext> _basicsServices { get; set; }
        ...
        public ICalibrationInstructionsLTIBase CalibrationInstructions { get; set; }
...
#pragma warning disable CS0109 
        ...
        public string demoEnabled { get; set; }
        ...
        protected override async Task OnInitializedAsync()
        {
            ...
            demoEnabled = (Customer == "Demo") ? "true" : "false";
        ...
        public async Task LoadWOD2()
        {
            ...
            else
            {
                    ...
                    || (EquipmentTypeObject?.CalibrationType?.CalibrationSubTypes == null|| EquipmentTypeObject?.CalibrationType?.CalibrationSubTypes.Count==0))
                {
                    ...
                    if (eq?.IsAccredited == false && result?.CalibrationSubTypes?.All(x=>x.DynamicPropertiesSchema != null)== true)
                    {
                ...
                if (eq?.WorkOder?.WorkOrderId > 0)
                {
                    ...
                    if (allWorkOrderDetails != null && allWorkOrderDetails.Count > 0)
                    {
                        ...
                        if (currentWod != null)
                        {
                            ...
                            listWods = listWods = filteredList.Where(x => x.PieceOfEquipment?.EquipmentTemplate?.EquipmentTypeObject?.EquipmentTypeGroupID == currentWod.PieceOfEquipment?.EquipmentTemplate?.EquipmentTypeObject?.EquipmentTypeGroupID).ToList();
                            ...
                            foreach (var item in listWods)
                            {
                                ...
                                {
                                    Key = item.WorkOrderDetailID.ToString(),
                                    Value = value
                                };
                            ...
                            if (compositesIds != null)
                            {
                                ...
                                {
                                    CalibrationSaaS.Domain.Aggregates.Entities.Customer customer = new Domain.Aggregates.Entities.Customer();
                                    customer.CustomerID = item;
                                    PieceOfEquipmentGRPC poegrpc = new PieceOfEquipmentGRPC(_poeServices, DbFactory, con.CallOptions);
                                    var poesByCustomer = await poegrpc.GetPieceOfEquipmentByCustomerId(customer);

                                    foreach (var poe in poesByCustomer.PieceOfEquipments.Where(x => x.EquipmentTemplate?.EquipmentTypeObject?.EquipmentTypeGroupID == currentWod.PieceOfEquipment?.EquipmentTemplate?.EquipmentTypeObject?.EquipmentTypeGroupID).ToList())
                                    {

                                        var value = poe?.PieceOfEquipmentID + "-" + poe?.CustomerId;
                                        KeyValueOption key = new KeyValueOption()
                                        {
                                            Key = poe?.PieceOfEquipmentID.ToString() ?? "0",
                                            Value = value
                                        };
                                        compositeWods.Add(key);
                                    }
        ...
        public async Task ShowTimeLine()
        {
            ...
            var messageForm = Modal.Show<WorkOrderDetailHist_Search>("TimeLine", parameters);
        ...
        public async Task ShowCertified()
        {
            ...
            var messageForm = Modal.Show<Certified_Search>("Certifies", parameters);
            var result = await messageForm.Result;
