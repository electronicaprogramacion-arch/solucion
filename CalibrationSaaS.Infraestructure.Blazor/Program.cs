...
namespace CalibrationSaaS.Infraestructure.Blazor
{
    public class Program
    {
        ...
        public static async Task Main(string[] args)
        {
            ...
            // Disable all logging to keep console clean
            ...
            builder.Services.AddLogging(loggingBuilder => loggingBuilder
                ...
                .AddFilter("Microsoft.EntityFrameworkCore", loglevel)
                .AddFilter("Microsoft.EntityFrameworkCore.Database.Command", loglevel)
                .AddFilter("Microsoft.EntityFrameworkCore.Query", loglevel)
                .AddFilter("Microsoft.EntityFrameworkCore.Model", loglevel)
            );
            ...
            if (!AppSecurity.IsReport)
            {
                ...
                builder.Services.AddTransient<Func<dynamic, Application.Services.IQuoteService<CallContext>>>(QuoteServiceFactorySqlite);

                builder.Services.AddTransient<Func<dynamic, Application.Services.IPriceTypeService<CallContext>>>(PriceTypeServiceFactorySqlite);
                ...
                builder.Services.AddTransient(services =>
                {
                    return CreateService<Application.Services.IAuditLogService<CallContext>>(AddressEnnpoint);
                });

                // Equipment Recall Service
                builder.Services.AddTransient(services =>
                {
                    return CreateService<Application.Services.IEquipmentRecallService<CallContext>>(AddressEnnpoint);
                });

                // Quote Service
                builder.Services.AddTransient(services =>
                {
                    return CreateService<Application.Services.IQuoteService<CallContext>>(AddressEnnpoint);
                });

                // Price Type Service
                builder.Services.AddTransient(services =>
                {
                    return CreateService<Application.Services.IPriceTypeService<CallContext>>(AddressEnnpoint);
                });
            ...
            // Add Entity Type Provider for audit logging
            builder.Services.AddScoped<CalibrationSaaS.Infraestructure.Blazor.Services.IEntityTypeProvider, CalibrationSaaS.Infraestructure.Blazor.Services.EntityTypeProvider>();

            // Add MET/CAL Integration service
            builder.Services.AddScoped<Application.Services.IMetCalIntegrationService<CallContext>, CalibrationSaaS.Infraestructure.Blazor.Services.MetCalIntegrationServiceGrpc>();
...
service =>
{
    return database =>
    {
        ...
        private static Func<IServiceProvider, Func<dynamic, Application.Services.IQuoteService<CallContext>>> QuoteServiceFactorySqlite =>
service =>
{
    return database =>
    {
        if (ConnectionStatusService.GetCurrentStatus())//online
        {
            ...
            return CreateService<Application.Services.IQuoteService<CallContext>>(ConnectionStatusService.grpcUrl);
        }
        else
        {
            //ILocalStorageService localStorageService = new LocalStorageService();
        ...
        private static Func<IServiceProvider, Func<dynamic, Application.Services.IPriceTypeService<CallContext>>> PriceTypeServiceFactorySqlite =>
service =>
{
    return database =>
    {
        if (ConnectionStatusService.GetCurrentStatus())//online
        {
            ...
            return CreateService<Application.Services.IPriceTypeService<CallContext>>(ConnectionStatusService.grpcUrl);
        }
        else
        {
            //ILocalStorageService localStorageService = new LocalStorageService();
