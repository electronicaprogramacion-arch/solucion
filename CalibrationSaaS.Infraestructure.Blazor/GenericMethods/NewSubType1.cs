...
{
    public class NewSubtype1 : ComponentBase, INewItem<WorkOrderDetail,GenericCalibration2>
    {
        ...
        public async Task<List<GenericCalibration2>> DefaultNewItem(WorkOrderDetail eq, int CalibrationSubTypeId)
        {
            ...
            {
                tespo = eq.PieceOfEquipment.TestPointResult.Where(x => x.CalibrationSubTypeId == Parameter.CalibrationSubtypeID.Value).ToList();
            }
            else
            {
