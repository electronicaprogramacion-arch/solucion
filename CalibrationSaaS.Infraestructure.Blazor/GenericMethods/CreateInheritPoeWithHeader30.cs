...
{
    public class CreateInheritPoeWithHeader30 : CreateBase<WorkOrderDetail, GenericCalibration2,GenericCalibrationResult2>, ICreateItems<WorkOrderDetail, GenericCalibration2>
    {
        public async Task<List<GenericCalibration2>> CreateItems(WorkOrderDetail eq, int CalibrationSubTypeId, string GroupName = null)
        {
            ...
            if (result != null && result.Count > 0 && tespo != null && tespo.Count > 0)
            {
                int cont = 0;
                foreach (var item in result)
                {
                    ...
                    foreach (PropertyDescriptor prop in TypeDescriptor.GetProperties(objparent))
                    {
                        Console.WriteLine("PROP: " + prop.Name);

                        if (objparent[prop.Name] != null)
                        {
                            obj[prop.Name] = objparent[prop.Name];//prop.GetValue(objparent);
                        }
                    ...
                    //obj.LT = lt;
                    //obj.UT = ut;
                    //obj.FunctionTested= objparent.FunctionTested;
