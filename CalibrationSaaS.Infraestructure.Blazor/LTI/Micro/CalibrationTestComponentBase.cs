...
using CalibrationSaaS.Infraestructure.Blazor.LTI.Micro;

namespace CalibrationSaaS.Infraestructure.Blazor
{
    public partial  class CalibrationTestComponent<CalibrationItem, CalibrationItemResult>  
         : CalibrationSaaS.Infraestructure.Blazor.GridResultsComponentBase<GenericCalibration> 
         //where CalibrationItem : class, IGenericCalibrationSubType<IResult2>, new() where CalibrationItemResult : class, IResult2, new()
    {
...
#pragma warning restore CS4014 // Como esta llamada no es 'awaited', la ejecución del método actual continuará antes de que se complete la llamada. Puede aplicar el operador 'await' al resultado de la llamada.
       ...
       public async  Task<List<WeightSet>> GetWeightSetList2(CalibrationItem item, int position)
        {


            List<WeightSet> list = new List<WeightSet>();   
            ...
            List<PieceOfEquipment> resultado = null;//(await _poeGrpc.GetPieceOfEquipmentByScale((Rockwell(item).BasicCalibrationResult.ScaleRange , Header));
        ...
        public async Task SelectWeight(CalibrationItem item, int position)
        {
            ...
            WeightSetList2 = await GetWeightSetList2(item, position);
            ...
            //TODO
            var messageForm = Modal.Show(typeof(CalibrationSaaS.Infraestructure.Blazor.LTI.Rockwell.WeightComponent<CalibrationItem>),"Select " + "" , parameters, op);
