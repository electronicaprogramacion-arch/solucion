...
namespace CalibrationSaaS.Infraestructure.Blazor.LTI.Leeb
{
    public class CreateItemsAsLeft : ComponentBase, ICreateItems<WorkOrderDetail, GenericCalibration2>
    {
        ...
        public async Task<List<GenericCalibration2>> CreateItems(WorkOrderDetail eq, int CalibrationSubTypeId, string GroupName = null)
        {
            ...
            //cal.BasicCalibrationResult = NewMethod(eq, CalibrationSubTypeId);
                ...
                foreach (var item in eq.WOD_Weights)
                {
                    ...
                    if (poe != null && poe.TestPointResult != null)
                    {
                        foreach(var poeitem in poe.TestPointResult)
                        {
                            ...
                            gce.Scale = poedyn.Scale;

                            gce.IN = poedyn.IN;
                            gce.SN = poedyn.SN;
                            gce.Value = poedyn.Value;
                            gce.Tolerance = poedyn.Tolerance;
                            gce.Uncertainty = poedyn.Uncertanty;
