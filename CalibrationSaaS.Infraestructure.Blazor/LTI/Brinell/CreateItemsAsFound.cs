...
namespace CalibrationSaaS.Infraestructure.Blazor.LTI.Brinell
{
    public class CreateItemsAsFound : ComponentBase, ICreateItems<WorkOrderDetail,GenericCalibration2>
    {
                ...
                foreach (var item in eq.WOD_Weights)
                {
                    ...
                    if (poe != null && poe.TestPointResult != null)
                    {
                        foreach (var poeitem in poe.TestPointResult)
                        {
                            ...
                            gce.BallDiam = poedyn.BallDiam;

                            gce.Load = poedyn.Load;
                            gce.LTI = poe.PieceOfEquipmentID;
                            gce.HBW = poedyn.HBW;
                            gce.DiamAvgTestBlock = poedyn.DiamAvg;
                            gce.Tolerance = poedyn.Tolerance;
                            gce.Uncertanty = poedyn.Uncertanty;
