...
namespace CalibrationSaaS.Infraestructure.Blazor.LTI.LaserMicrometer.RepeatabilityTest
{
    public class Get : ComponentBase, IGetItems<WorkOrderDetail,GenericCalibration2>
    {
        public async Task<List<GenericCalibration2>> GetItems(WorkOrderDetail eq, int CalibrationSubTypeId, int ihneritCalSubtypeId)
        {
            ...
            var items2 = await new CalibrationSaaS.Infraestructure.Blazor.LTI.LaserMicrometer.RepeatabilityTest.Create().CreateItems(eq, CalibrationSubTypeId);
