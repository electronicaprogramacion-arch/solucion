...
{
    public class LinearityComponentBase : KavokuComponentBase<WorkOrderDetail>
    {
        ...
        [Inject]
        public Func<dynamic, Application.Services.IPieceOfEquipmentService<CallContext>> PoeService { get; set; }
...
#pragma warning restore CS4014 // Como esta llamada no es 'awaited', la ejecución del método actual continuará antes de que se complete la llamada. Puede aplicar el operador 'await' al resultado de la llamada.
        ...
        {
            var Linearities = RT.Items;
            foreach (var lin in Linearities)
            {
                lin.BasicCalibrationResult.AsLeft = lin.BasicCalibrationResult.AsFound;
        ...
        public List<Domain.Aggregates.Entities.Linearity> LIST { get; set; } = new List<Domain.Aggregates.Entities.Linearity>();
...
#pragma warning restore CS4014 // Como esta llamada no es 'awaited', la ejecución del método actual continuará antes de que se complete la llamada. Puede aplicar el operador 'await' al resultado de la llamada.
        ...
        private void OnUserFinish(Object source, ElapsedEventArgs e)
        {
            ...
            InvokeAsync(async () =>
            {
                try
                {
                    ...
                    var aa = aa1.BalanceAndScaleCalibration.Linearities;
                    int cont = 0;
        ...
        public List<Domain.Aggregates.Entities.Linearity> FivePonints()
        {
        ...
        public List<Domain.Aggregates.Entities.Linearity> TenPonints()
        {
            List<Domain.Aggregates.Entities.Linearity> listlienarity = new List<Domain.Aggregates.Entities.Linearity>();

            for (int i = 0; i < 5; i++)
            {
                listlienarity.Add(new Domain.Aggregates.Entities.Linearity { SequenceID = i });
            }
        ...
        public async Task SelectWeight(Linearity item, int position)
        {
            ...
            var messageForm = Modal.Show<CalibrationSaaS.Infraestructure.Blazor.LTI.Scale.WeightComponent<Linearity>>("Select Weight", parameters, op);
            ...
            if (WeightSetList != null)
            {
                ...
                // FEATURE DISABLED: "Propagate weights to test points below"
                // This feature was confusing because it would automatically copy weights to all test points below the current one.
                // It also caused issues when clearing weights (would clear weights for test points below as well).
                // To re-enable this feature, uncomment the code block below.

                /*
                // OLD BEHAVIOR: Propagate weights to all test points below the current position
                ...
                */
                ...
                // If WeightSetList is empty and RemoveList has items, user clicked "Clear"
                // In this case, set WeightSets to an empty list (not null, not cleared)
                // This ensures the backend receives an empty list instead of NULL
                if (WeightSetList.Count == 0 && RemoveList.Count > 0)
                {
                    // Set to new empty list instead of clearing - this ensures it's not NULL when serialized
                    RT.Items[position].WeightSets = new List<WeightSet>();
                }
                else
                {
                    // Otherwise, remove only the specific weights in RemoveList
                    foreach (var ws in RemoveList)
                    {
                        var result11 = RT.Items[position].WeightSets.Where(x => x.PieceOfEquipmentID == ws.PieceOfEquipmentID && x.WeightSetID == ws.WeightSetID).FirstOrDefault();
                        if (result11 != null)
                        {
                            RT.Items[position].WeightSets.Remove(result11);
                        }
                    }
                }
                var decimalNumber = (int)WorkOrderItemCreate.eq.DecimalNumber;

                // FEATURE DISABLED: "Propagate WeightApplied to test points below"
                // This was part of the "propagate weights to test points below" feature.
                // To re-enable, uncomment the code block below.

                /*
                // OLD BEHAVIOR: Update WeightApplied for all test points below the current position
                for (int i = cont; i < RT.Items.Count; i++)
                {
                    double suma = 0;
                    //RT.Items[i].BasicCalibrationResult.WeightApplied = suma + CalResult;
                ...
                */
                ...
                // NEW BEHAVIOR: Only update the current test point (at position)
                double suma = 0;
                if (RT.Items[position].WeightSets != null && RT?.Items[position]?.TestPoint?.NominalTestPoit != 0)
                {
                    foreach (var ws in RT.Items[position].WeightSets)
                    {
                        suma = suma + ws.WeightNominalValue;
                    }

                    // Use the calculated sum from the current test point's WeightSets
                    RT.Items[position].BasicCalibrationResult.WeightApplied = Math.Round(suma, decimalNumber);
                }
                else
                {
                    RT.Items[position].BasicCalibrationResult.WeightApplied = 0;
                }
